...
```
✓ Installed: /Users/montimage/.claude/statusline.sh (v1.15.1)
✓ Installed: /Users/montimage/.local/bin/context-stats (v1.15.1)
✓ Python package installed: cc-context-stats==1.15.1
...
✗ Python package 'cc-context-stats' is not installed.
  Install it with: pip3 install cc-context-stats==1.15.1
```

If there's a version mismatch:
```
✗ Python package version mismatch:
    Script version:   1.15.1
...
Updated VERSION in `scripts/context-stats.sh` from 1.11.1 to 1.15.1 to match `pyproject.toml`.
...
- `scripts/context-stats.sh` — Updated `dispatch_python_subcommand()` with version checking, fixed VERSION to 1.15.1
